...
func TestRunGet(t *testing.T) {
	...
	if err := runGet(context.Background(), &buf, res, stubNetwork{n: testLAN()}, "2026-07-27"); err != nil {
...
func TestRunGetToleratesNetworkFailure(t *testing.T) {
	...
	var buf bytes.Buffer
	if err := runGet(context.Background(), &buf, res, stubNetwork{err: errors.New("nope")}, "2026-07-27"); err != nil {
		t.Fatalf("runGet error: %v", err)
	}
...
func TestRunGetPropagatesReservationFailure(t *testing.T) {
	...
	var buf bytes.Buffer
	if err := runGet(context.Background(), &buf, res, stubNetwork{n: testLAN()}, "2026-07-27"); err == nil {
		t.Error("runGet succeeded despite a reservation read failure")
	}
}
